...
NTSTATUS request_firmware(struct firmware** img, PCWSTR path) {
	*img = NULL;
	...
	struct firmware* fw = (struct firmware*)ExAllocatePoolZero(NonPagedPool, sizeof(struct firmware), MAX98390_POOL_TAG);
	if (!fw) {
	...
	fw->data = (UINT8*)ExAllocatePoolWithTag(NonPagedPool, fw->size, MAX98390_POOL_TAG);
	if (!fw->data) {
...
void free_firmware(struct firmware* fw) {
	if (fw->data) {
		ExFreePoolWithTag(fw->data, MAX98390_POOL_TAG);
	}
	ExFreePoolWithTag((PVOID)fw, MAX98390_POOL_TAG);
}